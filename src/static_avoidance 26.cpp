...
class StaticAvoidance{
    ...
    public:    
        ...
        void obstacle_cb(obstacle_detector::Obstacles raw_obstale){
            // 8월 26일 버전에 오른쪽 장애물 먼져 나오는 코드
            ...
            if (STATUS == 0){
                ...
                //else if (status_flag_ == false && nearest_segment_center_point_.x > 0.7) status_flag_ == true; 
            }
                ...
                acker_data.drive.steering_angle = (status_flag_ == false) ? MIN_STEERING+5 : MAX_STEERING-5;  // 장애물 중점이 왼쪽에 있다면 25 아니면 -25를 전달.
                cout<<"angle : " << angle << endl <<"tempAngle : " << tempAngle << endl;
                if ((abs(angle - tempAngle) >= ANGLE) || (status_flag_==true) && (SegmentVector.x<1.8)){           // 이전 tempAngle 과 현재 angle 값 비교
                    STATUS = 2;
