...
class StaticAvoidance{
    private:
        ...
        bool right_first_flag = true; // 왼쪽 부터오는 경우 status_right_flag를 flase로 설정
        float first_steering = 0;
        float second_steering = 0;
        ...
        obstacle_detector::Obstacles first_obstacle;
        obstacle_detector::Obstacles second_obstacle;
    ...
    public:    
        ...
        void first_obstacle_control(){
            if (right_first_flag == false){
                first_obstacle = left_obstacle;
                second_obstacle = right_obstacle;
                first_steering = MAX_STEERING ;
                second_steering = MIN_STEERING ;
                cout << "LEFT FIRST" << endl;
            }
            else{
                first_obstacle = right_obstacle;
                second_obstacle = left_obstacle;
                first_steering = MIN_STEERING;
                second_steering = MAX_STEERING;
        ...
        void obstacle_cb(obstacle_detector::Obstacles raw_obstacle){
            ...
            switch(KUUVe_STATUS)
            {
                case STATUS_GO:{
                    ...
                    findNearestSegment(first_obstacle);
                    ...
                    /*
                    else if (status_flag_ == false && abs(nearest_segment_center_point_.y) > 0.8) {// 첫번째 장애물을 그냥 지날칠 때, 첫번째 순환 생략
                ...
                case STATUS_TURN:{
                    ...
                    float segment_average_x = 0;
                    float obstacle_dist_1 = 0;
                    float temp_steering_angle = 0;
                    if(status_flag_ == true) cout << "status_flag_ : " << status_flag_ << endl;
                    
                    cur_obstacle = (status_flag_ == false) ? first_obstacle : second_obstacle;  // 삼항연산자를 이용하여 왼쪽에 있으면 left , 아니면 right로 구분.
                    
                    findNearestSegment(raw_obstacle);
                    
                    for (auto i : raw_obstacle.segments){
                    ...
                    segment_average_x = abs(segment.first_point.x +segment.last_point.x)/2;
                    cout << "segment_average_x : " << segment_average_x << endl; 
                    ...
                    obstacle_dist_1 = distance(nearest_segment_center_point_);
                    if (MAXDISTANCE < obstacle_dist_1) MAXDISTANCE = obstacle_dist_1;
                    
                    temp_steering_angle = (status_flag_ == false) ? MIN_STEERING : MAX_STEERING;  // 각도를 좀 줄였음.
                    acker_data.drive.steering_angle = temp_steering_angle;
                    cout << "steering angle : " << acker_data.drive.steering_angle << endl;

                    
                    cout<<"angle : " << angle << endl << "tempAngle : " << tempAngle << endl;
                    
                    if ((abs(angle - tempAngle) >= ANGLE)&&(status_flag_ ==false)){           // 이전 tempAngle 과 현재 angle 값 비교
                    ...
                    else if((abs(angle - tempAngle)>=ANGLE)&&(status_flag_ == true)){
                        KUUVe_STATUS = STATUS_LAST_RESTORE;
                        MAXDISTANCE = 0;
                        double time_status_1 = ros::Time::now().toSec(); 
                    }
                    /*
                    if ( segment_average_x<2.0 && status_flag_ == false)  // 조건문을 steering angle로 변경해도 될것 같기도? 혹은 거리를 감소하는 뱡항으로 해도 될것 같음.
                    { // TODO : 기준값을 물체마다 바꿔줘야함.
                        KUUVe_STATUS = STATUS_RESTORE;
                        cout << "average_y : " << segment_average_y << endl;
                        cout<<"***********************************"<<endl<<"CHANGE!!!!!"<<endl << "*******************************"<<endl;
                    }
                    else if (segment_average_x<2.0 && status_flag_ == true ){
                        KUUVe_STATUS = STATUS_LAST_RESTORE;
                        MAXDISTANCE = 0;
                        double time_status_1 = ros::Time::now().toSec();
                    }
                ...
                case STATUS_RESTORE:{
                    ...
                    int count_seg_2=0;
                    cur_obstacle = (status_flag_ == false) ? first_obstacle : second_obstacle;
                    ...
                    wayPoint.y = wayPoint.y/(count_seg_2);
                    ...
                    */
                    ...
                    if (wayPoint.x > 4 && status_flag_ == false) { // TODO 여기에 raw_obastacle의 x값을 넣어도 될것 같음 일단 테스트 해봐야함.
                        KUUVe_STATUS = STATUS_GO_NEXT;
                ...
                case STATUS_GO_NEXT:{
                    ...
                    else if (obstacle_dist_3 < 3.5 && status_flag_ == true) KUUVe_STATUS = STATUS_TURN;
                ...
                case STATUS_LAST_RESTORE:{
                    acker_data.drive.steering_angle = (right_first_flag == false) ? 15 : -15;
                    ros::Duration second_duration(2.0);
                    time_status_4 = ros::Time::now().toSec();
                    cout << "running time :" << time_status_4 << endl; 
                    if(time_status_4- time_status_1 > 4 ) KUUVe_STATUS = STATUS_FINISH; // 시간을 이용하여 STATUS 넘어감, 다른 방식 고려
...
int main(int argc, char **argv){
    ros::init(argc, argv, "static_avoidance_3");  
