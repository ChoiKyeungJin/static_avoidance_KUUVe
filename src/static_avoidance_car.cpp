...
typedef enum Status 
        {
            ...
            STATUS_TURN_RIGHT,  // assigned = 1
            STATUS_DETECT_LEFT, // assigned = 2
            STATUS_TURN_LEFT,   // assigned = 3
            STATUS_RESTORE,     // assigned = 4
            STATUS_FINISH,      // assigned = 5
        }STATUS;

class StaticAvoidance_car{
    private:
        ...
        // Const Values
        const float SeekDistance = 3.4;
        const float ANGLE = 15; // 원통이 1개여서 ANGLE값 15에서 8로 수정
        const float GAP = 2.5;
    ...
    public:    
        ...
        void start(){
            ROS_INFO("STATIC AVOIDANCE CAR START");   // ROS_INFO는 C/C++ 의 printf 나 cout과 같은 기능을 한다.
            obstacle_sub_left = nh_.subscribe("raw_obstacles_left", 10, &StaticAvoidance_car::obstacle_left_cb, this); 
            obstacle_sub_right = nh_.subscribe("raw_obstacles_right", 10, &StaticAvoidance_car::obstacle_right_cb, this);
            obstacle_sub_raw = nh_.subscribe("raw_obstacles", 10, &StaticAvoidance_car::obstacle_cb, this); 
        ...
        void obstacle_cb(obstacle_detector::Obstacles raw_obstacle){
            ...
            acker_data.drive.speed = 1.5;
            if(KUUVe_STATUS == STATUS_FINISH) acker_data.drive.speed = 0;
            // TODO:
            ...
            switch(KUUVe_STATUS)
            {
                case STATUS_GO:{
                    ...
                    //cur_SeekDistance = (status_flag_ == false) ? SeekDistance : SeekDistance - 0.5; 
                    if (obstacle_dist_0 <= SeekDistance && obstacle_dist_0 > 1.5) KUUVe_STATUS = STATUS_TURN_RIGHT;
                ...
                case STATUS_TURN_RIGHT:{
                    ...
                    steering_angle_ = (status_flag_ == false) ? MIN_STEERING-5 : MAX_STEERING+5;
                    ...
                    cout << "angle - tempAngle : "<< angle - tempAngle << endl;
                    ...
                    cout<<"SegmentVector.x : " << SegmentVector.x << endl;
                    cout << "STATUS : STATUS_1" << endl;
                    break;
                }
                ...
                case STATUS_DETECT_LEFT:{
                    ...
                    if (status_flag_ == true && acker_data.drive.steering_angle < -10){ 
                        KUUVe_STATUS = STATUS_RESTORE;
                        time_status_2 = ros::Time::now().toSec(); 
                    }
                    else if (wayPoint.x < 0.8 && wayPoint.x > 0.5&&status_flag_ == false) { // TODO 더 괜찮은 방식 조건 생각하기
                        KUUVe_STATUS = STATUS_TURN_LEFT;
...
int main(int argc, char **argv){
    ...
    StaticAvoidance_car staticAvoidance_car;        // 이 부분에서 생성자가 실행되고 start()가 작동함.

    while(ros::ok()){
        //staticAvoidance.exect();    // INIT==0 이면 직진
        staticAvoidance_car.acker_pub.publish(staticAvoidance_car.getAckerData());      // acker_data를 받아온다. speed, steering_angle
        staticAvoidance_car.visualization_waypoint();
